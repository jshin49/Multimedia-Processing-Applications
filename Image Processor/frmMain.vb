...
#End Region
    ...
    Public Class imageObject
        Public prevImage As Bitmap
        Public originalImage As Bitmap
        Public rotatedImage As Bitmap
        Public rotatedLayer As Bitmap
        Public currentAngle As Integer

        Public Sub Add(ByVal image As frmImage)
            currentAngle = 0
            originalImage = image.image.Clone()
            rotatedImage = originalImage.Clone()
            prevImage = originalImage.Clone()
        End Sub

        Public Sub Reset(ByVal image As Bitmap)
            originalImage = image.Clone()
            rotatedImage = image.Clone()
            rotatedLayer = Nothing
            currentAngle = 0
        End Sub

        Public Sub Close()
            If currentImageIndex = iObject.Length - 1 Then
                iObject(currentImageIndex) = Nothing
            Else
                For i As Integer = currentImageIndex To iObject.Length - 2
                    iObject(i) = iObject(i + 1)
                Next i
                iObject(iObject.Length - 2) = Nothing
            End If
            currentImageIndex = 0
        End Sub
    End Class
    ...
    Public Shared iObject(5) As imageObject
    Public Shared currentImageIndex As Integer = 0
    ...
    Public Sub mnuFileOpen_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuFileOpen.Click
        ...
        If dlg.ShowDialog() = DialogResult.OK Then
            ...
            While (Not iObject(counter) Is Nothing)
                counter += 1
            End While
            iObject(counter) = New imageObject
            If counter = iObject.Length Then ReDim Preserve iObject((iObject.Length - 1) * 2)
            iObject(counter).Add(selectedImageForm)
        End If
    End Sub
    ...
    Private Sub mnuReset_Click(sender As Object, e As EventArgs) Handles mnuReset.Click
        If Not selectedImageForm Is Nothing Then
            iObject(currentImageIndex).currentAngle = 0
            iObject(currentImageIndex).originalImage = iObject(currentImageIndex).prevImage.Clone()
            iObject(currentImageIndex).rotatedImage = iObject(currentImageIndex).prevImage.Clone()
