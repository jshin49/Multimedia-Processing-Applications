...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        'btnApply
        '
        Me.btnApply.Location = New System.Drawing.Point(171, 17)
        ...
        Me.ClientSize = New System.Drawing.Size(468, 67)
...
#End Region
    ...
    Overrides Function Process(ByRef image As Bitmap, ByVal x As Integer, ByVal y As Integer) As Color
        ...
        'Your code here
        Dim center As New Point(Math.Floor(kernel_width / 2.0), Math.Floor(kernel_height / 2.0))
        Dim red As Integer
        Dim green As Integer
        Dim blue As Integer
        Dim divisor As Double

        For i As Integer = x - center.X To (x - center.X) + kernel_width - 1
            For j As Integer = y - center.Y To (y - center.Y) + kernel_height - 1
                'modImage.outOfImage(image, i, j)
                If 0 <= i And i <= image.Width - 1 And 0 <= j And j <= image.Height - 1 Then
                    red += image.GetPixel(i, j).R * kernel(i - (x - center.X), j - (y - center.Y))
                    green += image.GetPixel(i, j).G * kernel(i - (x - center.X), j - (y - center.Y))
                    blue += image.GetPixel(i, j).B * kernel(i - (x - center.X), j - (y - center.Y))
                End If
                divisor += kernel(i - (x - center.X), j - (y - center.Y))
            Next j
        Next i
